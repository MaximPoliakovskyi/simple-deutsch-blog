...
function isNextDataCacheSizeError(error: unknown): boolean {
...
export async function fetchJson<T>(
  ...
  try {
    ...
    const send = async (requestPolicyInit: Pick<NextFetchInit, "cache" | "next">) =>
      fetch(input, {
        ...options.init,
        ...requestPolicyInit,
        next: mergeNext(requestPolicyInit.next, options.init?.next),
        headers,
    ...
    try {
      response = await send(policyInit);
    } catch (error) {
      if (policyWithLocale.type !== "DYNAMIC" && isNextDataCacheSizeError(error)) {
        console.warn(
          `[fetchJson] Falling back to dynamic fetch for oversized cached payload: ${String(input)}`,
        );
        response = await send(policyToInit({ type: "DYNAMIC" }));
      } else {
        throw error;
      }
    }
