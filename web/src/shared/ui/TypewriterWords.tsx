 ...
 * - Fixed-width stage to keep the headline centered as words change
...
export default function TypewriterWords({
  ...
  return (
    <>
      ...
      {/* Stage with fixed width; keeps the line centered and stable. */}
      <span
        className={`inline-flex items-baseline justify-center whitespace-nowrap align-baseline select-text ${containerClassName} ${className}`}
        style={{
          width: stableWidth,
        ...
        aria-hidden="true"
      >
        <span className="pointer-events-auto inline-flex min-h-[1em] items-baseline justify-center whitespace-nowrap select-text">
          <span className="inline-block select-text">{displayText || "\u00A0"}</span>
          {showCursor && !prefersReducedMotion ? (
            <span
              className="caret-realistic ml-px inline-block leading-[1] pointer-events-none select-none"
              aria-hidden="true"
            >
              |
            </span>
      ...
      <style
        ...
        @media (prefers-reduced-motion: reduce) {
          .caret-realistic { animation: none; }
        }
      `,
        }}
      />
