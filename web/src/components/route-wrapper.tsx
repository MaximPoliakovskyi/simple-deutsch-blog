...
function waitDoubleRaf(signal?: AbortSignal) {
  return new Promise<void>((resolve, reject) => {
    ...
    firstFrame = requestAnimationFrame(() => {
      secondFrame = requestAnimationFrame(() => { cleanup(); resolve(); });
    });
  });
}
...
function waitForMs(signal: AbortSignal, durationMs: number) {
  return new Promise<void>((resolve, reject) => {
    if (signal.aborted) { reject(createAbortError()); return; }
    const timeoutId = window.setTimeout(() => { cleanup(); resolve(); }, durationMs);
    const cleanup = () => { window.clearTimeout(timeoutId); signal.removeEventListener("abort", onAbort); };
    const onAbort = () => { cleanup(); reject(createAbortError()); };
...
export function RouteTransitionProvider({ children }: { children: ReactNode }) {
  ...
  const logDev = useCallback((event: string, extra: Record<string, unknown> = {}) => {
    if (!DEBUG) return;
    console.log("[route-transition]", { event, phase: phaseRef.current, token: activeTransitionRef.current?.token ?? 0, pathname: pathnameRef.current, targetPathname: activeTransitionRef.current?.targetPathname ?? null, ...extra });
  }, []);
  ...
  const clearReadySignals = useCallback(() => { readySignalsRef.current.clear(); }, []);

  const isCurrentTransition = useCallback((expectedToken: number) => {
    return activeTransitionRef.current?.token === expectedToken && activeRunRef.current?.token === expectedToken && !activeRunRef.current.controller.signal.aborted;
  }, []);
  ...
  const resetToIdle = useCallback((reason: string) => {
  ...
  const waitForPathnameMatch = useCallback((signal: AbortSignal, expectedToken: number, expectedPathname: string, timeoutMs: number) => {
    if (signal.aborted) return Promise.resolve(false);
    if (pathnameRef.current === expectedPathname && isCurrentTransition(expectedToken)) return Promise.resolve(true);
    return new Promise<boolean>((resolve) => {
      ...
      const waiterEntry: Waiter = { token: expectedToken, pathname: expectedPathname, settle: finish };
  ...
  const waitForRouteReadySignal = useCallback((signal: AbortSignal, expectedToken: number, expectedPathname: string, timeoutMs: number) => {
    if (signal.aborted) return Promise.resolve(false);
    const readySet = readySignalsRef.current.get(expectedToken);
    if (readySet?.has(expectedPathname) && isCurrentTransition(expectedToken)) return Promise.resolve(true);
    return new Promise<boolean>((resolve) => {
      let settled = false;
      let timeoutId = 0;
      const finish = (matched: boolean) => {
        ...
        window.clearTimeout(timeoutId);
        signal.removeEventListener("abort", onAbort);
        routeReadyWaitersRef.current = routeReadyWaitersRef.current.filter((w) => w !== waiterEntry);
        resolve(matched);
      };
      const onAbort = () => finish(false);
      const waiterEntry: Waiter = { token: expectedToken, pathname: expectedPathname, settle: finish };
  ...
  const runTransition = useCallback(async (activeTransition: ActiveTransition, signal: AbortSignal) => {
    ...
    try {
      ...
      if (pushedTokenRef.current !== activeTransition.token) {
        ...
        reactStartTransition(() => { router.push(activeTransition.href); });
      }
      setPhaseSafe("waiting_ready");
      logDev("waiting-ready", { token: activeTransition.token });
      const loadingStartedAt = readyStartAt();
      const pathnameMatched = await waitForPathnameMatch(signal, activeTransition.token, activeTransition.targetPathname, READY_MAX_MS);
      if (!isCurrentTransition(activeTransition.token)) return;
      if (pathnameMatched) {
        ...
        const readyMatched = await waitForRouteReadySignal(signal, activeTransition.token, activeTransition.targetPathname, remainingMs);
        if (!isCurrentTransition(activeTransition.token)) return;
        if (!readyMatched && process.env.NODE_ENV !== "production") {
          console.warn(`[route-transition] route-ready timeout for ${activeTransition.targetPathname} (token ${activeTransition.token})`);
        }
      }
      ...
      if (process.env.NODE_ENV !== "production") console.error("[route-transition] transition failed", error);
      if (!isCurrentTransition(activeTransition.token)) return;
      resetToIdle("error");
    }
  }, [isCurrentTransition, logDev, resetToIdle, router, setPhaseSafe, waitForPathnameMatch, waitForRouteReadySignal]);

  const beginTransition = useCallback(({ nextDirection = "forward", nextTargetHref, nextTargetPathname }: { nextDirection?: TransitionDirection; nextTargetHref: string; nextTargetPathname: string }) => {
    if (activeRunRef.current && !activeRunRef.current.controller.signal.aborted) { logDev("begin-blocked", { phase: phaseRef.current, reason: "run-active" }); return false; }
    if (phaseRef.current !== "idle") { logDev("begin-blocked", { phase: phaseRef.current }); return false; }
    const nextToken = tokenCounterRef.current + 1;
    tokenCounterRef.current = nextToken;
    cancelActiveRun("pre-begin-safety");
    clearWaiters();
    clearReadySignals();
    const nextTransition: ActiveTransition = { token: nextToken, href: nextTargetHref, targetPathname: nextTargetPathname, direction: nextDirection };
    ...
    lockScroll();
    logDev("begin", { token: nextToken, nextTargetHref, nextTargetPathname, direction: nextDirection });
  ...
  const navigateFromLogo = useCallback((href: string) => {
    if (!href) return false;
    const nextPathname = toPathname(href, pathnameRef.current);
    if (nextPathname === pathnameRef.current) return false;
    if (prefersReducedMotion) { router.push(href); return true; }
    return beginTransition({ nextDirection: "forward", nextTargetHref: href, nextTargetPathname: nextPathname });
  }, [beginTransition, prefersReducedMotion, router]);

  const navigateFromLanguageSwitch = useCallback((href: string) => {
    if (!href) return false;
    const nextPathname = toPathname(href, pathnameRef.current);
    if (nextPathname === pathnameRef.current) return false;
    if (prefersReducedMotion) { router.push(href); return true; }
  ...
  const signalRouteReady = useCallback((readyPathname: string, readyToken: number) => {
    ...
    for (const waiter of routeReadyWaitersRef.current) {
      if (waiter.token === readyToken && waiter.pathname === normalizedPathname) waiter.settle(true);
  ...
  useEffect(() => {
    return () => { resetToIdle("unmount"); forceUnlockScroll(); };
  }, [resetToIdle]);

  const contextValue = useMemo(() => ({
    phase, direction, isActive, token, pathname, targetPathname,
    navigateFromLogo, navigateFromLanguageSwitch, signalRouteReady,
  }), [direction, isActive, navigateFromLanguageSwitch, navigateFromLogo, pathname, phase, signalRouteReady, targetPathname, token]);
  ...
  return (
    <TransitionNavContext.Provider value={contextValue}>
      {children}
      {shouldRenderOverlay ? (
        <div aria-hidden="true" data-phase={phase} data-direction={direction} className="rt-overlay" style={overlayStyle}>
          <Image src="/main-logo.svg" alt="" width={220} height={220} className="rt-overlay__logo" priority />
        </div>
      ) : null}
...
export function useRouteReadySignal() {
  const pathname = normalizeRoutePathname(usePathname() || "/");
  const { token, signalRouteReady } = useTransitionNav();
  return useCallback(() => { signalRouteReady(pathname, token); }, [pathname, signalRouteReady, token]);
}
...
export function AppFadeWrapper({ children }: { children: ReactNode }) {
  ...
  useLayoutEffect(() => {
    if (phase !== "idle") {
      ...
      setFadeState((current) => (current === "hidden" || current === "fading-out" ? current : "fading-out"));
      return;
    }
    ...
    const firstFrame = requestAnimationFrame(() => {
      setFadeState("hidden");
      secondFrame = requestAnimationFrame(() => { setFadeState("fading-in"); });
    });
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => { setFadeState("visible"); wasTransitionActiveRef.current = false; }, 1000);
