    ...
    <div className="w-full max-w-232 mx-auto">
      {src ? (
        <div className={`aspect-video rounded-2xl overflow-hidden w-full ${className}`}>
          <Image src={src} alt={alt} width={928} height={Math.round((928 * 9) / 16)} className="w-full h-full object-cover" />
        </div>
      ) : (
        <div className={`aspect-video bg-neutral-200 dark:bg-white/10 rounded-2xl w-full ${className}`} />
      )}
    </div>
  )
}
