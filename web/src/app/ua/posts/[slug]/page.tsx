...
import { getPostBySlug, getPostsByCategorySlug, getPostsPageByCategory } from "@/lib/wp/api";
...
export async function generateMetadata({ params }: { params: ParamsPromise }): Promise<Metadata> {
  ...
  if (!post) return { title: TRANSLATIONS["ua"].postNotFound };
...
export default async function UaPostPage({ params }: { params: ParamsPromise }) {
  const { slug } = await params;
  ...
  async function findPostFallback(slugToCheck: string) {
    ...
    try {
      const page = await getPostsPageByCategory({ first: 50, categorySlug: "ua" });
  ...
  // Format date for Ukrainian locale (use 'uk-UA' for Intl)
  const formattedDate = date ? date.toLocaleDateString("uk-UA", { year: "numeric", month: "long", day: "numeric" }) : "";
  ...
  const currentLang = getPostLanguage(post) ?? "ua";
          ...
          {visibleCategories.length ? (
            <div className="mb-6 flex flex-wrap gap-2">
              {visibleCategories.map((cat) => (
                ...
                >
                  {translateCategory(cat!.name, cat!.slug, "ua")}
                </Link>
              ))}
            </div>
          ) : null}
              ...
              <ul className="text-sm text-gray-600 dark:text-gray-400">
                {moreArticles.map((p) => (
                  <li key={p.slug} className="py-4 border-b border-slate-200 dark:border-slate-700 last:border-0">
                    <Link href={`/ua/posts/${p.slug}`} className="hover:underline block">
                      {p.title}
                    </Link>
                  </li>
                ))}
